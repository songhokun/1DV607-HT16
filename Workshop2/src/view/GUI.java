...
public class GUI implements Initializable, IView {
	...
	public void simpleSearch(Object o) {
		...
		try {
			...
			ArrayList<Member> result = registry.simpleSearch(object);
			if (!result.isEmpty()) {
				displaySuccess(result.size() + " Result(s) Found!!");
		...
		boatEditColumn.setCellFactory(param -> new TableCell<Boat, Boat>() {
			...
			protected void updateItem(Boat boat, boolean empty) {
				...
				if (boat == null) {
					...
					boatEditButton.setOnAction(event -> updateBoat(member, boat.getLength(), boat.getType(), boat));
				}
			}
		});
